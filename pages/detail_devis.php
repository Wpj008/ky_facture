...
<head>
  ...
  <title><?= $devis['numero_devis'] ?> — KY-Factura</title>
...
<div class="app-shell">
  ...
  <div class="main">
    ...
    <main class="page">
      <nav class="cluster-2 mb-4 text-secondary text-small"><a href="devis.php" class="text-secondary">Devis</a><i class="bi bi-chevron-right text-xs"></i><span class="fw-medium" style="color:var(--color-text)"><?= $devis['numero_devis']; ?></span></span></nav>

      <div class="page-header">
        <div>
          <div class="cluster-2"><h1 class="mb-0"><?= $devis['numero_devis']; ?></h1><span class="badge-ds badge-devis-<?= $devis['id_status_devis']; ?>"><?= $devis['name_status_devis']; ?></span></div>
          <p class="mb-0">Émis le <?= date('d/m/Y', strtotime($devis['date_creation_devis'])); ?> · échéance le <?= date('d/m/Y', strtotime($devis['date_validite_devis'])); ?></p>
        </div>
        <div class="cluster-2">
        <a href="../impressions/imprime_devis.php?id_devis=<?= $devis_id ?>" class="btn btn-secondary"><i class="bi bi-download"></i> PDF</a>
   
          <a href="add_paiement.php?devis_id=<?= $devis['id_devis']; ?>" class="btn btn-success" ><i class="bi bi-send"></i> Envoyer</a>
        </div>
      </div>
        ...
        <div class="col-lg-8">
          <div class="card-ds" style="padding:var(--space-8)">
            <div class="d-flex justify-content-between flex-wrap gap-4 mb-5">
              ...
              <div class="text-end">
                <h2 class="mb-1">Devis</h2>
                <p class="text-secondary mb-0">N° <?= $devis['numero_devis']; ?><br>Date : <?= date('d/m/Y', strtotime($devis['date_creation_devis'])); ?><br>Échéance : <?= date('d/m/Y', strtotime($devis['date_validite_devis'])); ?></p>
              </div>
            </div>

            <div class="surface p-4 mb-5" style="background:var(--color-bg);border:0">
              <small class="eyebrow">Créé pour </small>
              <div class="fw-semibold mt-1" style="font-size:var(--fs-h5)"> Nom client : <?= $devis['firstname_customer'] . ' ' . $devis['lastname_customer']; ?></div>
              <p class="text-secondary mb-0"> Entreprise : <?= $devis['entreprise'] ?? 'Non spécifiée' ?></p>
              <p class="text-secondary mb-0"> Adresse : <?= $devis['adresse_customer']; ?><br>Email : <?= $devis['email_customer']; ?></p>
            </div>
            ...
            <table class="table-ds" style="border:1px solid var(--color-border);border-radius:var(--radius-lg);overflow:hidden">
              ...
              <tbody>
                ...
                <tr><td class="cell-strong"><?= $ligne['name_service']; ?></td><td><?= (int) $ligne['quantite_devis']; ?></td><td class="text-mono"><?= $ligne['prix_unitaire']; ?> €</td><td class="text-mono text-end"><?= $ligne['montant_ht']; ?> €</td></tr>
                <?php endforeach; ?>
              </tbody>
            </table>
            ...
            <div class="d-flex justify-content-end mt-4">
              <div style="min-width:280px">
                <div class="cluster-2 justify-content-between py-2"><span class="text-secondary">Total HT</span><span class="text-mono"><?= $devis['total_ht']; ?> €</span></div>
                <div class="cluster-2 justify-content-between py-2"><span class="text-secondary">TVA (20 %)</span><span class="text-mono"><?= $devis['total_tva']; ?> €</span></div>
                <hr class="divider my-2">
                <div class="cluster-2 justify-content-between py-2"><span class="fw-semibold" style="font-size:var(--fs-h5)">Total TTC</span><span class="text-mono fw-bold" style="font-size:var(--fs-h4);color:var(--color-primary)"><?= $devis['total_ttc']; ?> €</span></div>
              </div>
            </div>
        ...
        <div class="col-lg-4">
          ...
          <div class="card-ds">
            ...
            <ul class="list-unstyled mb-0 stack-3">
              <li class="d-flex gap-3"><span class="kpi__icon kpi__icon--info" style="width:32px;height:32px"><i class="bi bi-send"></i></span><div>Devis envoyée<div><small class="text-muted-2">28 juin · 14h02</small></div></div></li>
